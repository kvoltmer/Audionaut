                      ...
                      const WaveformEnvelope* envelope)
    {
             ...
             && isPositiveAndBelow (channelNum, numChannelsCached))
        {
            ...
            if (! clip.isEmpty())
            {
                ...
                juce::Path thePath;
                thePath.preallocateSpace(clip.getWidth()*2*3 + 9);
                
                cacheData = getData (channelNum, clip.getX() - area.getX());
                x = (float) clip.getX();
                
                // first point is at zero crossing
                thePath.startNewSubPath (juce::Point<float> (x, midY));
                ...
                // make sure last point is at zero crossing
                thePath.lineTo(x, midY);
;
                // LOWER PART (we go backwards: --cacheData)
                --cacheData;
                for (int w = clip.getWidth(); --w >= 0;)
                {
                    if (cacheData->isNonZero())
                ...
                g.strokePath(thePath, PathStrokeType (0.5f));
            }
        }
    }
