...
public:
    ...
    void updateVisibleArea (const bool makeSureItUpdatesContent)
    {
        ...
        auto newH = 0;
        for (auto r = 0; r < owner.totalItems; r++)
        {
            if (auto* m = owner.getModel())
    ...
    void updateContents()
    {
        ...
        // let's go for all components
        const size_t numNeeded = owner.totalItems;
        ...
        for (auto row = 0; row < rows.size(); ++row)
        {
            ...
            {
                auto rowH = owner.model->getRowHeight(row);
        ...
        /* this is the original code that only updates the conent on screen
        if (rowH > 0)
        {
